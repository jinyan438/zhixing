...
export default defineConfig(({ mode }) => {
  // Keep standalone Vite runs aligned with the root .env used by the backend.
  const env = loadEnv(mode, path.resolve(__dirname, '..'), '')
  const backendHost = process.env.BACKEND_HOST || env.BACKEND_HOST || env.HOST || '127.0.0.1'
  const proxyHost = ['0.0.0.0', '::'].includes(backendHost) ? '127.0.0.1' : backendHost
  const backendPort = process.env.BACKEND_PORT || env.BACKEND_PORT || process.env.PORT || env.PORT || '3118'
